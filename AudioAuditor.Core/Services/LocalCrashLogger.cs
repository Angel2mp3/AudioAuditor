...
namespace AudioQualityChecker.Services
{
    ...
    public static class LocalCrashLogger
    {
        ...
        {
            try { return AudioAuditorSettings.CurrentThemeName ?? "(unknown)"; }
            catch { return "(unknown)"; }
        }
        ...
        public static void ClearAllLogs()
        {
            ...
            {
                if (!Directory.Exists(CrashLogDir)) return;
                foreach (var f in Directory.GetFiles(CrashLogDir, "crash-*.txt"))
                    try { File.Delete(f); } catch { }
            }
            catch { }
        }

        public static (int count, long totalBytes) GetLogStats()
        {
            try
            {
                if (!Directory.Exists(CrashLogDir)) return (0, 0);
                var files = Directory.GetFiles(CrashLogDir, "crash-*.txt");
