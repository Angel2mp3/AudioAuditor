...
namespace AudioQualityChecker.Services
{
    ...
    public readonly struct DecoderResult
    {
        ...
        /// <summary>
        /// Releases every decoder this result owns. Several fields deliberately alias the same
        /// object (opus/vorbis/dsd/flac are stored as both ExtraDisposable and WaveStreamReader),
        /// so distinct instances are tracked by reference to avoid double-dispose. Conversely,
        /// some paths own genuinely separate objects — step 7 holds an MfReader *and* a
        /// WaveFormatConversionStream (an ACM handle), step 10 a WaveFileReader *and* its
        /// converter — so disposing only the first non-null field leaks the rest.
        /// Wrappers are released before the streams they wrap.
        /// </summary>
        public void Dispose()
        {
            // Wrappers first, then the streams they wrap.
            var owned = new IDisposable?[] { ExtraDisposable2, ExtraDisposable, WaveStreamReader, MfReader, Reader };
            for (int i = 0; i < owned.Length; i++)
            {
                ...
                for (int j = 0; j < i; j++)
                    if (ReferenceEquals(owned[j], item)) { alreadyReleased = true; break; }
