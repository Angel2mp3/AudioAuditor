...
namespace AudioQualityChecker.Services
{
    ...
    public static class LocalStatsCollector
    {
        private static readonly string StatsFile = Path.Combine(
            ...
            "AudioAuditor", "stats.json.gz");

        // Pre-compression filename. Read once on upgrade, then deliberately LEFT IN PLACE and never
        // written to again: unlike the scan cache, listening history cannot be regenerated, so this
        // frozen copy is what a user who rolls back to an older build falls back on. Do not "clean
        // it up" — it stops a downgrade from silently zeroing someone's Wrapped stats.
        ...
        // Every recorded event used to rewrite the whole file synchronously, and
        // RecordAnalysisResult runs once per analysed file on the UI thread — so a 10,000-file scan
        // meant 10,000 full serialise-and-write passes. Events now just mark the data dirty and
        // Flush() persists at the points that matter (scan end, app exit, periodic tick).
        private static bool _dirty;

        /// <summary>Minimum gap between automatic flushes triggered by recorded events.</summary>
        private static readonly TimeSpan AutoFlushInterval = TimeSpan.FromMinutes(2);
        ...
        public static bool IsEnabled => AudioAuditorSettings.StatsCollectionEnabled;
        ...
        public static void Reset()
        {
            ...
            {
                _data = new StatsData();
                // Written straight through rather than left to the debounce: "clear my stats" is
                // destructive and must survive a crash or a kill immediately after.
                _dirty = true;
                Save();

                // The frozen pre-compression copy has to go too, otherwise a reset user who ever
                // runs an older build gets the stats they just deleted handed back to them.
                try { if (File.Exists(LegacyStatsFile)) File.Delete(LegacyStatsFile); } catch { }
            }
        }
        ...
        {
            // Falls back to the pre-compression stats.json when the gzipped file isn't there yet,
            // so upgrading keeps every play, scan and analysis already recorded.
            return CompressedJsonStore.Load<StatsData>(StatsFile, LegacyStatsFile) ?? new StatsData();
        }

        /// <summary>
        /// Marks the collected data as needing a write, and persists it only if enough time has
        /// passed since the last one. Callers hold <see cref="_lock"/>.
        /// </summary>
        private static void MarkDirty()
        {
            _dirty = true;

            if (DateTime.UtcNow - _lastFlushUtc >= AutoFlushInterval)
                Save();
        }

        /// <summary>
        /// Writes pending stats to disk. Safe and cheap to call when nothing changed. Call at the
        /// end of a scan and on app shutdown so nothing recorded since the last flush is lost.
        /// </summary>
        ...
        private static void Save()
        {
            ...
            // Gzipped on top of that — this file is ~85% smaller than the plain JSON it replaces.
            if (!CompressedJsonStore.Save(StatsFile, _data, SaveOptions)) return;

            _dirty = false;
            _lastFlushUtc = DateTime.UtcNow;

            // The legacy stats.json is intentionally NOT deleted — see LegacyStatsFile.
        }
    }
